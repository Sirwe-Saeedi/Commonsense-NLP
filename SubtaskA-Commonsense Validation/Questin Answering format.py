# -*- coding: utf-8 -*-
"""TaskA_warmupsteps=320,_maxsteps=5336,_loggingsteps=200,savesteps=3400,lr=1e_5,_acc=96_08.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/13-91zVNmoQHvfsm_x5ha06PkianuOJen
"""
...
for i in range(len(df_data)):
    if df_data['sent0'][i][-1] != '.' or df_data['sent1'][i][-1] !='.' :
      df_data['sent0'][i] = df_data['sent0'][i] +'.'
      df_data['sent1'][i] = df_data['sent1'][i] +'.'
...
df_answers = pd.read_csv('subtaskA_gold_answers.csv', error_bad_lines=False, names=['id', 'label'])
...
for i in range(len(df_test)):
    if df_test['sent0'][i][-1] != '.' or df_test['sent1'][i][-1] !='.' :
      df_test['sent0'][i] = df_test['sent0'][i] +'.'
      df_test['sent1'][i] = df_test['sent1'][i] +'.'
    ...
    def _create_examples(self, lines: List[List[str]], type: str):
        ...
        examples = [
            InputExample(
                ...
                endings=[line[2], line[3]],
                #label=str(ord(line[-1]) - ord("A")),
                label = str(line[4])
            )
            for line in lines[1:]  # we skip the line with the column names
        ]
        print('examples[0].example_id', examples[1].example_id)
        print('examples[0].question', examples[1].question)
        print('examples[0].contexts', examples[1].contexts)
        print('examples[0].endings', examples[1].endings)
        print('examples[0].label', examples[1].label)
...
MULTIPLE_CHOICE_TASKS_NUM_LABELS = {"semeval", 2}
...
parser.add_argument(
    "--output_dir",
    default="output/checkpoint-3400",
    type=str,
    required=False,
...
parser.add_argument("--warmup_steps", default=320, type=int, help="Linear warmup over warmup_steps.")

parser.add_argument("--logging_steps", type=int, default=200, help="Log every X updates steps.")
parser.add_argument("--save_steps", type=int, default=3400, help="Save checkpoint every X updates steps.")
parser.add_argument(
...
import pandas as pd
subtaskA_answers = pd.DataFrame(predictions , columns= ['answer'])
subtaskA_answers.index = np.arange(1, len(subtaskA_answers) + 1)

id_of_test = df_test['id'].to_list()
id_of_test = pd.DataFrame(id_of_test , columns=['id'])
id_of_test.index = np.arange(1, len(id_of_test) + 1)
subtaskA_answers = id_of_test.join(subtaskA_answers)
subtaskA_answers

subtaskA_answers.to_csv('subtaskA_answers.csv', header=False , index=False)
...
"""#the following cells gave me incorrect result in post evaluation"""

df_test

df_test = df_test[['id' , 'prediction']]
df_test.to_csv('subtaskA_answers.csv' , index=False, header=False)
